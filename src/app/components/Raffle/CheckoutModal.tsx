          ...
          <Dialog.Portal forceMount>
            ...
            <Dialog.Content asChild>
              <motion.div
                ...
                className="fixed top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[90vw] md:w-[32rem] max-w-[95vw] h-[88vh] max-h-[900px] bg-card text-foreground rounded-3xl shadow-2xl p-0 z-50 focus:outline-none flex flex-col overflow-hidden border border-border"
              >
                <div className="flex-none px-5 py-3 border-b border-border flex items-center justify-between bg-muted/60">
                ...
                <div className="flex-1 min-h-0 p-5 md:p-6 space-y-6 overflow-hidden flex flex-col">
                  ...
                  <section className="flex-none bg-muted rounded-xl border border-border p-4">
                    <h3 className="flex items-center gap-2 text-foreground font-semibold text-sm mb-3">
                    ...
                    <dl className="space-y-2 text-sm text-muted-foreground">
                      ...
                      <div className="flex flex-wrap gap-2 items-center">
                        <dt className="text-muted-foreground shrink-0 w-full text-sm">Números ({selectedNumbers.length})</dt>
                        <dd className="flex flex-wrap gap-2">
                          ...
                          {selectedNumbers.length > 12 && (
                            <span className="inline-flex items-center justify-center h-8 px-2 rounded-xl font-semibold text-xs text-muted-foreground bg-background/60 border border-border">
                    ...
                    <div className="mt-3 pt-3 border-t border-border flex justify-between items-center">
                      <span className="font-semibold text-foreground text-sm">Total a pagar</span>
                  ...
                  <section className="flex-1 min-h-0 flex flex-col">
                    <h3 className="text-foreground font-semibold text-sm mb-3 flex-none">Datos del usuario</h3>
                    <form id="checkout-form" onSubmit={handleSubmit(onSubmit)} className="space-y-3 flex-1 min-h-0 flex flex-col">
                      <div>
                        <label className="block text-xs font-medium text-muted-foreground mb-1">Nombre y apellido</label>
                        <input
                          {...register("fullName", { required: "Este campo es requerido" })}
                          className="w-full px-3 py-2.5 text-sm rounded-lg border border-border bg-background focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all"
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          <label className="block text-xs font-medium text-muted-foreground mb-1">Red social</label>
                          <input
                ...
                <div className="flex-none px-5 py-3 border-t border-border bg-card flex gap-2 justify-end">
                  <button
                    type="submit"
                    form="checkout-form"
